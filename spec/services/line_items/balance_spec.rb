...
RSpec.describe LineItems::Balance, type: :service do
  let(:quantity) { 4 }
  let(:price) { 6 }
  ...
  describe 'Success response' do
    it 'should return balance after two_per_one discount successfully' do
      two_per_one
      new_total = (quantity - (quantity % 2)) / 2 * line_item.product.price
      service = LineItems::Balance.call(line_item: line_item)

      expect(service.success?).to be(true)
      expect(service.data[:balance][:total]).to eq(new_total)
    end
  end

  it 'should return balance after drop_unit_price discount successfully' do
    drop_unit_price
    service = LineItems::Balance.call(line_item: line_item)

    expect(service.success?).to be(true)
    expect(service.data[:balance][:total]).to eq(quantity * 4.5)
  end

  it 'should return balance after drop_total_price discount successfully' do
    drop_total_price
    service = LineItems::Balance.call(line_item: line_item)
    ...
    expect(service.data[:balance][:total].round(2)).to eq(new_total)
  end

  describe 'Failure response' do
    it 'shoud fail balance by missing line_item' do
      service = LineItems::Balance.call(line_item: nil)
